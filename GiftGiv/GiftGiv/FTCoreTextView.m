...
- (CGSize)suggestedSizeConstrainedToSize:(CGSize)size
{
	...
	if ([self isSystemUnder3_2]) {
		...
		suggestedSize = [_processedString sizeWithFont:style.font constrainedToSize:CGSizeMake(self.bounds.size.width, size.height)];
	}
	else {
...
- (void)fitToSuggestedHeight:(CGFloat)targetHeight
{
	CGSize suggestedSize = [self suggestedSizeConstrainedToSize:CGSizeMake(CGRectGetWidth(self.frame), targetHeight)];
	...
	if ([self isSystemUnder3_2]) {
		...
		if (drawFrame == NULL) {
			...
			if(YES){
                ...
                drawFrame = CTFramesetterCreateFrame(_framesetter, CFRangeMake(0, CFAttributedStringGetLength((CFAttributedStringRef)self.attributedString)), framePath, NULL);
                ...
                CTFrameGetLineOrigins(drawFrame, range, origins);
                ...
                rng.length = CFAttributedStringGetLength((CFAttributedStringRef)self.attributedString) - rng.location;
                // substring with that range
                CFAttributedStringRef longString = CFAttributedStringCreateWithSubstring(NULL, (CFAttributedStringRef)self.attributedString, rng);
                ...
                //[self drawImagesForFrame:drawFrame fromAttributedString:(__bridge NSAttributedString *)attrString];
...
{    
    ...
    NSArray *lines = (NSArray *)CTFrameGetLines(ctframe);
    NSInteger lineCount = 5;//[lines count];
    CGPoint origins[lineCount];
