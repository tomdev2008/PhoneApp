...
@implementation ValidationOfTheTokenRequest

@synthesize validateTokenDelegate;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    statusResponse=[[NSMutableString alloc]init];
	
	//delegate method to send the response after parsing finished successfully
	if([xmlParser parse]){
        
		[validateTokenDelegate statusOfValidation:statusResponse];
	}
	[statusResponse  release];
	[xmlParser release];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
	...
	[validateTokenDelegate requestFailed];
	[webData release];
	[theConnection release];
}
...
-(void) parser:(NSXMLParser*) parser didEndElement:(NSString*) argElementName namespaceURI:(NSString*) argNamespaceURI qualifiedName:(NSString*) argQualifiedName
{
	...
	if([argElementName isEqualToString:@"UpdateAccessTokenValidityResult"]){
        statusResponse=currentElementValue;
    }
