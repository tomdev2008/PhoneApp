...
#import "PostOnFacebookWallRequest.h"

@implementation PostOnFacebookWallRequest
@synthesize postOnFacebookReqDelegate;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    postOnWallResponse=[[NSMutableString alloc]init];
	
	//delegate method to send the response after parsing finished successfully
	if([xmlParser parse]){
        
		[postOnFacebookReqDelegate responseForPosting:postOnWallResponse];
	}
	[postOnWallResponse  release];
	[xmlParser release];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
	...
	[postOnFacebookReqDelegate requestFailed];
	[webData release];
	[theConnection release];
}
...
-(void) parser:(NSXMLParser*) parser didEndElement:(NSString*) argElementName namespaceURI:(NSString*) argNamespaceURI qualifiedName:(NSString*) argQualifiedName
{
	...
	if([argElementName isEqualToString:@"PostOnFacebookWallResult"]){
        postOnWallResponse=currentElementValue;
    }
